            ...
            >
              {mainImage && (
                <Image
                  ...
                  fill
                  priority
                  className="object-contain pd-main-img"
                  style={isZoomed
          ...
          <div className="pd-info-col">
                  ...
                  selectedVariation.original_price > selectedVariation.price && (
                    <>
                      ...
                      <span className="pd-discount-badge">
                        {Math.round(((selectedVariation.original_price - selectedVariation.price) / selectedVariation.original_price) * 100)}% OFF
                      </span>
                    </>
                  )}
              </div>
                ...
                <div className="pd-stars">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      size={16}
                      className={i < Math.floor(Number(calculateAvgRating())) ? "pd-star--filled" : "pd-star--empty"}
                    />
                  ))}
                </div>
                <span className="pd-rating-text">
                  {calculateAvgRating()} ({reviews.length} review{reviews.length !== 1 ? "s" : ""})
                </span>
              </div>
            </div>
              ...
              <button
                onClick={() => {
                  ...
                  addToCart({
                    ...
                    variationName: selectedVariation?.formatted_quantity || "",
            ...
            {product.stock > 0 && (
              <p className="pd-stock-note">
                <Package size={14} /> {product.stock} units available
              </p>
            )}
            ...
            <div className="pd-delivery-block">
              ...
              {deliveryInfo && (
                <div className="pd-delivery-result">
                  <Truck size={14} />
                  <span>Expected by <strong>{deliveryInfo.date}</strong></span>
                </div>
              )}
            </div>
            ...
            <div className="pd-trust-row">
              <div className="pd-trust-item">
                <span>Taxes and duties included and calculated at checkout</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="pd-reviews-layout">
            <div className="pd-review-form-block">
              ...
              <div className="pd-review-form">
                  ...
                  <div className="pd-stars-input">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button key={star} onClick={() => setReviewForm({ ...reviewForm, rating: star })}>
                        <Star
                          size={28}
                          className={star <= reviewForm.rating ? "pd-star--filled" : "pd-star--empty"}
                        />
                      </button>
                    ))}
                  </div>
                ...
                />
                <button onClick={submitReview} className="pd-review-submit">
                  Submit Review
                </button>
              </div>
            </div>
                        ...
                        <div className="pd-stars">
                          {[...Array(5)].map((_, j) => (
                            <Star
                              key={j}
                              size={15}
                              className={j < r.rating ? "pd-star--filled" : "pd-star--empty"}
                            />
                          ))}
                        </div>
                      </div>
            ...
            <div className="pd-related-grid">
              {relatedProducts.slice(0, 4).map((item) => (
                <div
                  ...
                  className="pd-related-card group"
                >
